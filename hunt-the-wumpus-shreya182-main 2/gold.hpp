#ifndef GOLD_HPP
#define GOLD_HPP

#include <string>
#include "event.hpp"
#include "game.hpp"

class gold : public Event {
public:
    /*
	 * Function: percept
	 * Description: Returns percept message for event 
	 * Returns (string): "You see something shimmer nearby"
	 */
    std::string percept() const { return "You see something shimmer nearby."; }
    /*
	 * Function: encounter
	 * Description: Performs action when user encounters object
	 * Parameters:
	 * 		game (class game object): reference to game object
	 * Returns (bool): True if object should stay in game
	 */
    bool encounter(Game& game);
    /*
	 * Function: get_debug_symbol
	 * Description: returns symbol to print on board 
	 * Returns (char): G to represent gold on board
	 */
    char get_debug_symbol() const { return 'G'; } 
};

#endif
